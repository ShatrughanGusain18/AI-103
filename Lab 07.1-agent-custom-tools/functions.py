            ...
            if len(parts) == 4:
                ...
                events.append(
                    (
                        parts[0],  # Event name
                        parts[1],  # Event type
                        month * 100 + day,  # Sortable month-day integer
                        parts[2],  # Original month-day string
                        set(parts[3].split(";")),  # Visible locations
                    )
                )
...
# ---------------------------------------------------------
# Get the next visible astronomical event
# ---------------------------------------------------------
...
def calculate_observation_cost(
    ...
    if tier not in TELESCOPE_RATES:
        return json.dumps(
            {
                "error": (
                    f"Unknown telescope tier '{telescope_tier}'. "
...
def generate_observation_report(
    ...
    Returns:
        JSON string containing the generated report filename.
    """
